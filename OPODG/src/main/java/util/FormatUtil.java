 ...
 * Revision 1.7  2013/12/26 08:59:09  이종건
 * System.out.println 조치
 *
 * Revision 1.6  2013/12/26 05:41:36  이종건
 * fortify조치(IP하드코딩 금지)
 *
 * Revision 1.5  2013/08/22 05:21:28  이종건
 * 2013-08-22 serverside source sync
 *
 * Revision 1.10  2013/07/09 01:02:51  brkim
 * 20130709 keb sync
 *
 * Revision 1.4  2012/02/08 06:58:30  신정섭
 * *** empty log message ***
 *
 * Revision 1.5  2012/02/08 06:59:41  moon
 * 포멧유틸 lpadding,rpadding 수정
 *
 ...
 * lastsync
 *
 * Revision 1.2  2011/01/05 01:33:35  이종건
 * getGmtToday 추가
 *
 * Revision 1.1  2009/10/07 06:03:40  김성수
 * *** empty log message ***
 *
 * Revision 1.5.2.5  2009/04/01 00:22:16  최지원
 * parseYYYYMMDD에 공백을 넣으면
 * 1111-11-11이 아니라 공백으로 return되도록 수정
 *
 ...
 * SpiderFramework 프로젝트 merge
 *
 * Revision 1.5  2008/08/19 00:59:26  shkim
 * 외환은행 CommonAssist 프로젝트와 동기화
 *
 * Revision 1.2  2007/06/28 02:07:20  안경아
 * *** empty log message ***
 *
...
public class FormatUtil {
    ...
    /**
    * 오늘 일자를 8자리(년월일:yyyyMMdd)의 문자열로 돌려준다. <BR><BR>
    *
    * 사용예) getToday()<BR>
    * 결 과 ) 20020930<BR><BR>
    *
    ...
    */
	...
	/**
	 * 문자열을 2만큼 뒤에서 소수점으로 처리해서 실수형을 금전형식을 리턴
	 * 맨 앞자리에 부호가 - 일 경우 -리턴. 양수면 삭제후 리턴 문자열에 소수점이 없을때만 사용
	 * 사용예) 
	 * getIndexFloatMoney("1234000) 결과 : 12,340.00
	 * getIndexFloatMoney("+1234010) 결과 : 12,340.10
	 * getIndexFloatMoney("-1234078) 결과 : -12,340.78
	 * 
	 * @param str : String
	 * @return String
	 */
     ...
     * 그러므로 '#'자체는 사용이 불가능하다.
     * 암호화 처리 되야 할 문자는 *로 표현한다( ex)******-#######)
     * 입력받은 문자가 Format보다 길다면 Format길이 이후의 문자열은 잘리게 된다.
     * Format이 입력받은 문자열보다 길다면 문자열만큼만 출력된다.
     *
     * @param pInstr 포맷팅 전 문자String
     * @param pInformat 포탯형식String(######-######} 등
     * @return String
     */
    ...
    */
	...
	public static String lPadding(String src, int size, char paddingChar) {
		...
		if (size == srcLength) {
			...
			// >>> * 2011-12-07 문철옹 : 실제 문자열보다 padding 길이가 작은경우 전각문자가 ? 로 표시되는 문제가 있어 수정함.
			boolean isFront = isFront2ByteString(srcBytes, size);	// 전각(2byte문자)의 경우 현위치가 전각의 앞부분인지 아닌지를 판단
			if(isFront) return paddingChar + new String(srcBytes, 0, size-1);	// 전각문자앞부분이면 버리고, SPACE처리
	...
	/**
	 * 2byte문자의 경우 index의 위치가 전각문자의 앞/뒤를 판단
	 * @param str
	 * @param size
	 * @return
	 */
	private static boolean isFront2ByteString(byte[] srcBytes, int index) {
		...
		for (int i = 0; i < index; i++) {
			byte now = srcBytes[i];
			if(((int)now & 0x80) != 0) {	// 0x80!=0   -> 2byte문자 판단(ascii경우 0을 리턴)
				isFront = isFront ? false : true;
				//if(isFront) System.out.print("앞");//else 		  System.out.print("뒤");
			}
		}
		return isFront;
	}
	...
	public static String rPadding(String src, int size, char paddingChar) {
		...
		// >>> * 2011-12-07 문철옹 : 실제 문자열보다 padding 길이가 작은경우 전각문자가 ? 로 표시되는 문제가 있어 수정함.
			...
			boolean isFront = isFront2ByteString(srcBytes, size);	// 전각(2byte문자)의 경우 현위치가 전각의 앞부분인지 아닌지를 판단
			if(isFront) return new String(srcBytes, 0, size-1) + paddingChar;	// 전각문자앞부분이면 버리고, SPACE처리
			else 		return new String(srcBytes, 0, size);
		}
		// <<< * 수정 끝. 
	...
	/**
	 * padding 처리
	 * @param src padding할 문자열
	 * @param size 문자열의 총길이
	 * @param paddingChar Padding할 문자
...
//	}
	 ...
	 * 주어진 문자열의 앞부분에 있는 '0'문자를 제거한다.
	 * 호스트로부터 전달받은 CICS COMMAREA의 숫자 필드인 경우에 앞부분에 채워진 '0'을 제거하기위해 사용될 수 있다.
	 * '0'을 제거한 문자열이 ''일 경우에 default값을 넘겨준다
	 *
	 * @param str String    앞에 0이 채워진 (오른쪽 정렬된) 문자열
	 * @param deflt         default 값
	 * @return String       앞에 채워진 '0'이 제거된 문자열 '0'을 제거한 문자열이 ''일 경우에 default값을 넘겨준다
	 */ 
...
//		System.out.println(rPadding("가", 4, '0'));
//		System.out.println(rPadding("가", 3, '0'));
//		System.out.println(lPadding("가나", 1, '0'));
//		System.out.println(lPadding("가나", 6, '0'));
